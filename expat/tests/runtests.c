...
record_cdata_nodefault_handler(void *userData,
                     ...
                     int UNUSED_P(len))
{
    ElementData_AddData((ElementData *)userData, NULL, CDATA_NODEFAULT, 0, 0);
}
...
START_TEST(test_default_current)
{
    ...
    const char *entity_text =
        ...
        "<doc>&entity;</doc>";
    ElementData storage;
    const ElementResults expected1 = {
        12,
        {
            { "", DEFAULT, 0, 0 },
            { "", CDATA, 0, 0 },
    ...
    const ElementResults expected2 = {
        7,
        {
            { "", DEFAULT, 0, 0 },
            { "", CDATA_NODEFAULT, 0, 0 },
    ...
    const ElementResults expected3 = {
        19,
        {
            { "", DEFAULT, 0, 0 },
            { "", DEFAULT, 0, 0 },
    ...
    const ElementResults expected4 = {
        20,
        {
            { "", DEFAULT, 0, 0 },
            { "", DEFAULT, 0, 0 },
    ...
    const ElementResults expected5 = {
        19,
        {
            { "", DEFAULT, 0, 0 },
            { "", DEFAULT, 0, 0 },
...
START_TEST(test_set_base)
{
    ...
    if (TSTR_CMP(XML_GetBase(parser), new_base) != 0)
...
static void
run_ns_tagname_overwrite_test(const char *text,
                              const ElementResults *result)
{
    ElementData storage;
    ElementData_Init(&storage);
    XML_SetUserData(parser, &storage);
...
START_TEST(test_ns_tagname_overwrite)
{
    ...
    const ElementResults result = {
        8,
        {
            { "http://xml.libexpat.org/ e",     START, 0, 0 },
            { "http://xml.libexpat.org/ f",     START, 0, 0 },
            { "http://xml.libexpat.org/ attr",  ATTRIBUTE, 0, 0 },
            { "http://xml.libexpat.org/ f",     END, 0, 0 },
            { "http://xml.libexpat.org/ g",     START, 0, 0 },
            { "http://xml.libexpat.org/ attr2", ATTRIBUTE, 0, 0 },
            { "http://xml.libexpat.org/ g",     END, 0, 0 },
            { "http://xml.libexpat.org/ e",     END, 0, 0 }
        }
    };
...
START_TEST(test_ns_tagname_overwrite_triplet)
{
    ...
    const ElementResults result = {
        8,
        {
            { "http://xml.libexpat.org/ e n",     START, 0, 0 },
            { "http://xml.libexpat.org/ f n",     START, 0, 0 },
            { "http://xml.libexpat.org/ attr n",  ATTRIBUTE, 0, 0 },
            { "http://xml.libexpat.org/ f n",     END, 0, 0, },
            { "http://xml.libexpat.org/ g n",     START, 0, 0 },
            { "http://xml.libexpat.org/ attr2 n", ATTRIBUTE, 0, 0 },
            { "http://xml.libexpat.org/ g n",     END, 0, 0 },
            { "http://xml.libexpat.org/ e n",     END, 0, 0 }
        }
    };
    XML_SetReturnNSTriplet(parser, XML_TRUE);
    run_ns_tagname_overwrite_test(text, &result);
